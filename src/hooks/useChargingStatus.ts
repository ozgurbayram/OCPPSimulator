...
export function useChargingStatus(cpId: string) {
  ...
  const latest = useMemo(() => {
    ...
    // Find the latest outbound MeterValues CALL for active connector
    const mv = frames.find(
      (f: OCPPFrame & any) => f.type === 'CALL' && f.dir === 'out' && f.action === 'MeterValues' && (f.raw?.[3]?.connectorId ?? connectorId) === connectorId,
    )
    const payload: any | undefined = mv?.raw?.[3]
    ...
    if (cp?.status === 'connected' && cp?.runtime?.transactionId != null) isCharging = true
    if (chargingData?.meterValue?.[0]?.sampledValue) {
      ...
      const current = samples.find((s) => s.measurand === 'Current.Offered' || s.measurand === 'Current.Import')
      ...
      if (powerKW > 0.1) isCharging = true
    }
